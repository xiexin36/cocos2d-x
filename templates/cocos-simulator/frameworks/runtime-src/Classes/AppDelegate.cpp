...
bool AppDelegate::applicationDidFinishLaunching()
{
    ...
    //
#if ((CC_TARGET_PLATFORM != CC_PLATFORM_WIN32) && (CC_TARGET_PLATFORM != CC_PLATFORM_MAC) && (COCOS2D_DEBUG > 0))
...
void AppDelegate::reopenProject()
{
    ...
    //
    //fileUtils->setSearchRootPath(_project.getProjectDir().c_str());
...
void StartupCall::trackEvent(const char *eventName)
{
    ...
    request->addPOSTValue("tid", "UA-57525059-1");
    request->addPOSTValue("cid", player::Device::getInstance()->getUserGUID().c_str());
    request->addPOSTValue("t", "event");
    
    request->addPOSTValue("an", "simulator");
    request->addPOSTValue("av", getRuntimeVersion());
...
//void StartupCall::onPreviewGAF(const std::string &path)
//{
//    CCLOG("------------------------------------------------");
//    CCLOG("LOAD GAF FILE: %s", path.c_str());
//    CCLOG("------------------------------------------------");
//    
//    cocos2d::Scene *scene = GafFeatures::create(path);
//
//    if (Director::getInstance()->getRunningScene())
//    {
//        Director::getInstance()->replaceScene(scene);
...
void StartupCall::updatePreviewFuncForPath(const std::string &path)
{
    ...
    else if (endWithString(path, ".gaf"))
    {
       // _previewFunc = std::bind(&StartupCall::onPreviewGAF, this, std::placeholders::_1);
       // setLoader(std::bind(&StartupCall::onPreviewGAF, this, std::placeholders::_1));
    }
}
